...
const getNBAChampions = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Error al obtener los campeones de la NBA' });
    }
};
...
const getSeasonStatsById = async (req, res) => {
    try {
        ...
        const query = `SELECT * FROM teams_stats WHERE idteam = $1 AND year = $2`;
...
const getPlayersStatsById = async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        const season = req.params.season;
        const query = `SELECT * FROM players_stats as ps JOIN teams as ts ON ps.team = ts.abreviation WHERE ts.id = $1 AND ps.season = $2`;
        const result = await pool.query(query, [id, season.toString()]);
